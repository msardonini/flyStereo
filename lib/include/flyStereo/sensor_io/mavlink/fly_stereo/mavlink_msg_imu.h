...
static inline uint16_t mavlink_msg_imu_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                                            uint64_t timestamp_us, uint32_t time_since_trigger_us,
                                            uint32_t trigger_count, float roll, float pitch, float yaw,
                                            const float* gyroXYZ, const float* accelXYZ) {
  ...
  return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_IMU_MIN_LEN, MAVLINK_MSG_ID_IMU_LEN,
                                  MAVLINK_MSG_ID_IMU_CRC);
}
...
static inline uint16_t mavlink_msg_imu_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan,
                                                 mavlink_message_t* msg, uint64_t timestamp_us,
                                                 uint32_t time_since_trigger_us, uint32_t trigger_count, float roll,
                                                 float pitch, float yaw, const float* gyroXYZ, const float* accelXYZ) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_IMU_LEN];
  _mav_put_uint64_t(buf, 0, timestamp_us);
  _mav_put_uint32_t(buf, 8, time_since_trigger_us);
  ...
  msg->msgid = MAVLINK_MSG_ID_IMU;
  return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_IMU_MIN_LEN,
                                       MAVLINK_MSG_ID_IMU_LEN, MAVLINK_MSG_ID_IMU_CRC);
}
 ...
 */
static inline uint16_t mavlink_msg_imu_encode(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                                              const mavlink_imu_t* imu) {
  return mavlink_msg_imu_pack(system_id, component_id, msg, imu->timestamp_us, imu->time_since_trigger_us,
                              imu->trigger_count, imu->roll, imu->pitch, imu->yaw, imu->gyroXYZ, imu->accelXYZ);
}
 ...
 * @param imu C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_imu_encode_chan(uint8_t system_id, uint8_t component_id, uint8_t chan,
                                                   mavlink_message_t* msg, const mavlink_imu_t* imu) {
  return mavlink_msg_imu_pack_chan(system_id, component_id, chan, msg, imu->timestamp_us, imu->time_since_trigger_us,
                                   imu->trigger_count, imu->roll, imu->pitch, imu->yaw, imu->gyroXYZ, imu->accelXYZ);
}
...
static inline void mavlink_msg_imu_send(mavlink_channel_t chan, uint64_t timestamp_us, uint32_t time_since_trigger_us,
                                        uint32_t trigger_count, float roll, float pitch, float yaw,
                                        const float* gyroXYZ, const float* accelXYZ) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  char buf[MAVLINK_MSG_ID_IMU_LEN];
  ...
  _mav_put_float_array(buf, 28, gyroXYZ, 3);
  _mav_put_float_array(buf, 40, accelXYZ, 3);
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_IMU, buf, MAVLINK_MSG_ID_IMU_MIN_LEN, MAVLINK_MSG_ID_IMU_LEN,
                                  MAVLINK_MSG_ID_IMU_CRC);
#else
  mavlink_imu_t packet;
  packet.timestamp_us = timestamp_us;
  ...
  mav_array_memcpy(packet.gyroXYZ, gyroXYZ, sizeof(float) * 3);
  mav_array_memcpy(packet.accelXYZ, accelXYZ, sizeof(float) * 3);
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_IMU, (const char*)&packet, MAVLINK_MSG_ID_IMU_MIN_LEN,
                                  MAVLINK_MSG_ID_IMU_LEN, MAVLINK_MSG_ID_IMU_CRC);
#endif
}
...
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
  mavlink_msg_imu_send(chan, imu->timestamp_us, imu->time_since_trigger_us, imu->trigger_count, imu->roll, imu->pitch,
                       imu->yaw, imu->gyroXYZ, imu->accelXYZ);
#else
  _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_IMU, (const char*)imu, MAVLINK_MSG_ID_IMU_MIN_LEN,
                                  MAVLINK_MSG_ID_IMU_LEN, MAVLINK_MSG_ID_IMU_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_imu_send_buf(mavlink_message_t* msgbuf, mavlink_channel_t chan, uint64_t timestamp_us,
                                            uint32_t time_since_trigger_us, uint32_t trigger_count, float roll,
                                            float pitch, float yaw, const float* gyroXYZ, const float* accelXYZ) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline float mavlink_msg_imu_get_roll(const mavlink_message_t* msg) { return _MAV_RETURN_float(msg, 16); }
 ...
 */
static inline float mavlink_msg_imu_get_pitch(const mavlink_message_t* msg) { return _MAV_RETURN_float(msg, 20); }
 ...
 */
static inline float mavlink_msg_imu_get_yaw(const mavlink_message_t* msg) { return _MAV_RETURN_float(msg, 24); }
