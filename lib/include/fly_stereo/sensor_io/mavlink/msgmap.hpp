...
class MsgMap {
 public:
  explicit MsgMap(mavlink_message_t *p) : msg(p), cmsg(p), pos(0) {}

  explicit MsgMap(mavlink_message_t &p) : msg(&p), cmsg(&p), pos(0) {}

  explicit MsgMap(const mavlink_message_t *p) : msg(nullptr), cmsg(p), pos(0) {}
...
template <typename _T>
typename std::enable_if<std::is_floating_point<_T>::value, typename UintBuffer<_T>::Type>::type to_little_endian(
    _T data) {
...
typename std::enable_if<std::is_floating_point<_TOutput>::value, _TOutput>::type to_host_from_little_endian(
    _TInput data) {
