...
#define MAVLINK_MAX_PACKET_LEN \
  (MAVLINK_MAX_PAYLOAD_LEN + MAVLINK_NUM_NON_PAYLOAD_BYTES + MAVLINK_SIGNATURE_BLOCK_LEN)  ///< Maximum packet length
 ...
 * mavlink_param_union_double_t will be 8 bytes long, and treated as needing 8 byte alignment for the purposes of
 * MAVLink 1.0 field ordering. The mavlink_param_union_double_t will be treated as a little-endian structure.
 *
 * If is_double is 1 then the type is a double, and the remaining 63 bits are the double, with the lowest bit of the
 * mantissa zero. The intention is that by replacing the is_double bit with 0 the type can be directly used as a double
 * (as the is_double bit corresponds to the lowest mantissa bit of a double). If is_double is 0 then mavlink_type gives
 * the type in the union. The mavlink_types.h header will also need to have shifts/masks to define the bit boundaries in
 * the above, as bitfield ordering isn’t consistent between platforms. The above is intended to be for gcc on x86, which
 * should be the same as gcc on little-endian arm. When using shifts/masks the value will be treated as a 64 bit
 * unsigned number, and the bits pulled out using the shifts/masks.
 */
