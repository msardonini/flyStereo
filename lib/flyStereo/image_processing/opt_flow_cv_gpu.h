...
#include "flyStereo/image_processing/opt_flow_cv_stream.h"
#include "opencv2/core/cuda.hpp"
#include "opencv2/cudaoptflow.hpp"

class OptFlowCvGpu : public OptFlowBase<OptFlowCvGpu, OptFlowCvStream> {
 public:
  using stream_type = OptFlowCvStream;
  ...
  void calc(const cv::cuda::GpuMat& prev_image, const cv::cuda::GpuMat& curr_image, const cv::cuda::GpuMat& prev_pts,
            cv::cuda::GpuMat& curr_pts, cv::cuda::GpuMat& status, OptFlowCvStream* stream = nullptr) {
    if (opt_flow_ptr_) {
      if (stream) {
  ...
  void calc(const UMat<uint8_t>& prev_image, const UMat<uint8_t>& curr_image, const UMat<cv::Vec2f>& prev_pts,
            UMat<cv::Vec2f>& curr_pts, UMat<uint8_t>& status, OptFlowCvStream* stream = nullptr) {
