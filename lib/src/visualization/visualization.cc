...
Visualization::Visualization(const YAML::Node &stereo_calibration) :
  ...
  cube_widget_(cv::Point3f(0.5,0.5,0.5), cv::Point3f(0.0,0.0,0.0), true, cv::viz::Color::blue())
 {
  ...
  Eigen::Matrix3d R_imu_cam0_eigen =
    Eigen::AngleAxisd(interface_vec[0], Eigen::Vector3d::UnitX()) *
  ...
  cv::Vec3f cam_pos(10.0f,-20.0f,5.0f), cam_focal_point(9.9f, -19.0f, 4.75f), cam_y_dir(0.0f,0.0f,
    -1.0f);
...
int Visualization::ReceiveData(const Eigen::Matrix4d &body_pose,
  const std::vector<cv::Point3d> &inliers) {
  cv::Matx33d rot_mat;
  cv::Vec3d trans_vec;
  ...
  if (!writer_) {
    writer_ = std::make_unique<cv::VideoWriter>("viz.mjpg", cv::VideoWriter::fourcc('M','J','P','G'), 11, viz_frame.size(), true);
  }
