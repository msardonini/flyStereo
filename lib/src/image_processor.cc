...
void ImageProcessor::Init() {
  ...
  d_opt_flow_cam0_ = cv::cuda::SparsePyrLKOpticalFlow::create(cv::Size(window_size_, window_size_), max_pyramid_level_,
                                                              max_iters_, true);
  d_opt_flow_stereo_t0_ = cv::cuda::SparsePyrLKOpticalFlow::create(cv::Size(window_size_, window_size_),
                                                                   max_pyramid_level_, max_iters_, true);
  d_opt_flow_stereo_t1_ = cv::cuda::SparsePyrLKOpticalFlow::create(cv::Size(window_size_, window_size_),
                                                                   max_pyramid_level_, max_iters_, true);
...
int ImageProcessor::UpdatePointsViaImu(const std::vector<cv::Point2f> &current_pts, const cv::Matx33d &rotation,
  ...
  cv::convertPointsToHomogeneous(current_pts, interface_pts);
  std::for_each(interface_pts.begin(), interface_pts.end(), [&H](auto &pt) { pt = H * pt; });
  cv::convertPointsFromHomogeneous(interface_pts, updated_pts);
...
int ImageProcessor::OuputTrackedPoints(OpticalFlowPoints &points, const std::vector<unsigned int> &ids,
                                       ...
                                       const cv::Matx33f &rotation_t0_t1_cam0, ImagePoints &output_points) {
  ...
  output_points.timestamp_us =
      std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch())
          .count();
  output_points.pts.clear();
  output_points.pts.reserve(num_pts);
  ...
  for (size_t i = 0; i < num_pts; i++) {
    ...
    output_points.pts.push_back(pt);
  }

  // Also save a copy of the imu messages that are associated to this camera frame
  output_points.imu_pts = imu_msgs;
  cv::Matx33d temp_rot = rotation_t0_t1_cam0;
  cv::cv2eigen(temp_rot, output_points.R_t0_t1_cam0);
  return 0;
}

int ImageProcessor::process_image(const cv::cuda::GpuMat &d_frame_cam0_t1, const cv::cuda::GpuMat &d_frame_cam1_t1,
  ...
  points_.AppendGpuMatColwise(d_c0_t0, t_c0_t0);
  points_.AppendGpuMatColwise(d_c1_t0, t_c1_t0);
  points_.ids[ids_t0].insert(std::end(points_.ids[ids_t0]), std::begin(points_.ids[ids_t1]),
                             std::end(points_.ids[ids_t1]));

  if (points_[t_c0_t0].cols > 2) {
    // Vector to indicate whether a point should be deleted or kept after binning
    std::vector<unsigned char> bin_status;
    points_.BinAndMarkPoints(t_c0_t0, ids_t0, d_frame_cam0_t1.size(), bins_width_, bins_height_, max_pts_in_bin_,
                             bin_status);
    if (!points_.RemoveOutliers(bin_status, {t_c0_t0, t_c1_t0}, {ids_t0})) {
      spdlog::info("status size {}", bin_status.size());
      spdlog::info("t_c0_t0 size {}", points_.GetCpu(t_c0_t0).size());
  ...
  if (retval == 0) {
    UpdatePointsViaImu(points_[t_c0_t0], rotation_t0_t1_cam0, K_cam0_, points_[t_c0_t1]);
    UpdatePointsViaImu(points_[t_c1_t0], rotation_t0_t1_cam1, K_cam1_, points_[t_c1_t1]);
  } else {
   ...
   *********************************************************************/
  if (!points_[t_c0_t1].empty() != 0 && !points_[t_c0_t0].empty()) {
    // // DEBUG CODE
    ...
    //     points_[t_c0_t0].download(pts_t0);
    //   }
    ...
    d_opt_flow_cam0_->calc(d_frame_cam0_t0_, d_frame_cam0_t1, points_[t_c0_t0], points_[t_c0_t1], d_status);
    ...
    if (points_[t_c0_t1].empty()) {
      points_[t_c0_t0].release();
      points_[t_c0_t1].release();
      d_status.release();
    }

    if (!points_.MarkPointsOutOfFrame(d_frame_cam0_t1.size(), t_c0_t1, d_status)) {
    ...
    // the second argument
    if (!points_.RemoveOutliers(d_status, {t_c0_t0, t_c0_t1, t_c1_t0}, {ids_t0})) {
    ...
    int ret_val =
        StereoMatch(d_opt_flow_stereo_t0_, d_frame_cam0_t1, d_frame_cam1_t1, points_, {t_c0_t1, t_c1_t1}, d_status);
    // Remove the outliers from the StereoMatch algorithm
    if (ret_val == 0) {
      if (!points_.RemoveOutliers(d_status, {t_c0_t0, t_c0_t1, t_c1_t0, t_c1_t1}, {ids_t0})) {
        spdlog::info("RemoveOutliers failed after StereoMatch");
        return -1;
      }
    }
    // debug_num_pts[2] = points_[t_c0_t1].cols;

    // Perform a check to make sure that all of our tracked vectors are the same length,
    // otherwise something is wrong
    int tracked_pts = points_[t_c0_t0].cols;
    if (tracked_pts != points_[t_c0_t1].cols || tracked_pts != points_[t_c1_t0].cols ||
        tracked_pts != points_[t_c1_t1].cols) {
   ...
   *********************************************************************/
  DetectNewFeatures(detector_ptr_, d_frame_cam0_t1, points_[t_c0_t1], points_[d_c0_t1]);
  // debug_num_pts[4] = points_[d_c0_t1].cols;
  // Match the detected features in the second camera
  points_[d_c1_t1].release();
  cv::cuda::GpuMat d_status;
  StereoMatch(d_opt_flow_stereo_t1_, d_frame_cam0_t1, d_frame_cam1_t1, points_, {d_c0_t1, d_c1_t1}, d_status);

  if (points_[d_c0_t1].cols != 0) {
    if (!points_.MarkPointsOutOfFrame(d_frame_cam0_t1.size(), d_c1_t1, d_status)) {
      spdlog::info("d status: {}", d_status.cols);
      spdlog::info("vec: {}", points_[d_c1_t1].cols);
      spdlog::info("MarkPointsOutOfFrame failed after detection");
      return -1;
    }

    if (!points_.RemoveOutliers(d_status, {d_c0_t1, d_c1_t1})) {
      spdlog::info("d status: {}", d_status.cols);
      spdlog::info("vec: {}", points_[d_c0_t1].cols);
      spdlog::info("vec: {}", points_[d_c1_t1].cols);
      spdlog::info("RemoveOutliers failed after Detection");
      return -1;
    }
    // debug_num_pts[5] = points_[d_c0_t1].cols;
  }
  t_det = std::chrono::system_clock::now();

  // Fill in the ids vector with our new detected features
  points_.ids[ids_t1].clear();
  points_.ids[ids_t1].reserve(points_[d_c0_t1].cols);

  for (int i = 0; i < points_[d_c0_t1].cols; i++) {
    points_.ids[ids_t1].push_back(current_id++);
  }

  // spdlog::info("num points: cam0 " << points_[1).cols << " cam1 " <<
  //   points_[3).cols << " start: " << debug_num_pts[0] << " tracking: " <<
  ...
  if (points_.GetCpu(t_c0_t1).size() > 1 && points_.GetCpu(t_c1_t1).size() > 1) {
    OuputTrackedPoints(points_, points_.ids[ids_t0], imu_msgs, rotation_t0_t1_cam0, output_points);
  }

  // Set the current t1 values to t0 for the next iteration
  d_frame_cam0_t0_ = d_frame_cam0_t1;
  // d_frame_cam0_t1.release();
  d_frame_cam1_t0_ = d_frame_cam1_t1;
  // d_frame_cam1_t1.release();
  points_[t_c0_t0] = points_[t_c0_t1].clone();
  points_[t_c1_t0] = points_[t_c1_t1].clone();
  points_[d_c0_t0] = points_[d_c0_t1].clone();
  points_[d_c1_t0] = points_[d_c1_t1].clone();
...
int ImageProcessor::StereoMatch(cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> opt, const cv::cuda::GpuMat &d_frame_cam0,
  ...
  if (!points.MarkPointsOutOfFrame(d_frame_cam1.size(), indices.second, d_status)) {
    spdlog::info("MarkPointsOutOfFrame failed 2");
    return -1;
  }
...
auto ImageProcessor::GetInputMaskFromPoints(const cv::cuda::GpuMat &d_input_corners, const cv::Size frame_size,
                                            cv::Mat &mask) -> int {
...
template <typename T>
int ImageProcessor::DetectNewFeatures(const cv::Ptr<T> &detector_ptr, const cv::cuda::GpuMat &d_frame,
