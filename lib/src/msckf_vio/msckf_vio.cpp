...
#include <fly_stereo/msckf_vio/math_utils.hpp>
// #include <fly_stereo/msckf_vio/utils.h>
...
MsckfVio::MsckfVio() : is_gravity_set(false), is_first_img(true) { return; }

bool MsckfVio::loadParameters(const YAML::Node& input_params) {
  input_params_ = input_params;
  // Frame id
  fixed_frame_id = input_params["fixed_frame_id"].as<std::string>();             // default "world"
  child_frame_id = input_params["child_frame_id"].as<std::string>();             // default "robot"
  publish_tf = input_params["publish_tf"].as<bool>();                            // default true
  frame_rate = input_params["frame_rate"].as<double>();                          // default 40.0
  position_std_threshold = input_params["position_std_threshold"].as<double>();  // default 8.0

  rotation_threshold = input_params["rotation_threshold"].as<double>();            // default 0.2618
  translation_threshold = input_params["translation_threshold"].as<double>();      // default 0.4
  tracking_rate_threshold = input_params["tracking_rate_threshold"].as<double>();  // default 0.5

  // Feature optimization parameters
  Feature::optimization_config.translation_threshold =
      input_params["feature_translation_threshold"].as<double>();  // Default 0.2;

  // Noise related parameters
  YAML::Node noise = input_params["noise"];
  IMUState::gyro_noise = noise["gyro"].as<double>();            // Default 0.001
  IMUState::acc_noise = noise["acc"].as<double>();              // Default 0.01
  IMUState::gyro_bias_noise = noise["gyro_bias"].as<double>();  // Default 0.001
  IMUState::acc_bias_noise = noise["acc_bias"].as<double>();    // Default 0.01
  Feature::observation_noise = noise["feature"].as<double>();   // Default 0.01
  ...
  YAML::Node initial_covariance = input_params["initial_covariance"];
  velocity_cov = initial_covariance["velocity"].as<double>();    // Defaul 0.25
  gyro_bias_cov = initial_covariance["gyro_bias"].as<double>();  // Defaul 1e-4
  acc_bias_cov = initial_covariance["acc_bias"].as<double>();    // Defaul 1e-2

  double extrinsic_rotation_cov, extrinsic_translation_cov;
  extrinsic_rotation_cov = initial_covariance["extrinsic_rotation_cov"].as<double>();        // 3.0462e-4
  extrinsic_translation_cov = initial_covariance["extrinsic_translation_cov"].as<double>();  // 1e-4
  ...
  std::vector<double> R_imu_cam0_euler_v = input_params["R_imu_cam0_euler"].as<std::vector<double> >();
  std::vector<double> T_imu_cam0_euler_v = input_params["T_imu_cam0"].as<std::vector<double> >();
  Eigen::Matrix3d R_imu_cam0_euler =
      (AngleAxisd(R_imu_cam0_euler_v[0], Vector3d::UnitX()) * AngleAxisd(R_imu_cam0_euler_v[1], Vector3d::UnitY()) *
       AngleAxisd(R_imu_cam0_euler_v[2], Vector3d::UnitZ()))
          .toRotationMatrix();
  Eigen::Vector3d T_imu_cam0_euler(T_imu_cam0_euler_v.data());

  Isometry3d T_imu_cam0 = Eigen::Isometry3d::Identity();
  T_imu_cam0.rotate(R_imu_cam0_euler);
  T_imu_cam0.translate(T_imu_cam0_euler);
  ...
  std::vector<double> R_cam0_cam1_euler_v = input_params["R_cam0_cam1"].as<std::vector<double> >();
  std::vector<double> T_cam0_cam1_euler_v = input_params["T_cam0_cam1"].as<std::vector<double> >();
  Eigen::Matrix3d R_cam0_cam1_euler(R_cam0_cam1_euler_v.data());
  Eigen::Vector3d T_cam0_cam1_euler(T_cam0_cam1_euler_v.data());

  CAMState::T_cam0_cam1 = Isometry3d::Identity();
  CAMState::T_cam0_cam1.rotate(R_cam0_cam1_euler);
  CAMState::T_cam0_cam1.translate(T_cam0_cam1_euler);

  IMUState::T_imu_body = Isometry3d::Identity();
  ...
  printf("initial velocity: %f, %f, %f\n", state_server.imu_state.velocity(0), state_server.imu_state.velocity(1),
...
bool MsckfVio::initialize(const YAML::Node& params) {
  if (!loadParameters(params)) return false;
...
void MsckfVio::imuCallback(const mavlink_imu_t& msg) {
...
void MsckfVio::initializeGravityAndBias() {
  ...
  while (!imu_msg_buffer.empty()) {
    ...
    angular_vel << imu_msg.gyroXYZ[0], imu_msg.gyroXYZ[1], imu_msg.gyroXYZ[2];
    linear_acc << imu_msg.accelXYZ[0], imu_msg.accelXYZ[1], imu_msg.accelXYZ[2];
  ...
  state_server.imu_state.gyro_bias = sum_angular_vel / 200;
  // IMUState::gravity =
  //  -sum_linear_acc / imu_msg_buffer.size();
  // This is the gravity in the IMU frame.
  Vector3d gravity_imu = sum_linear_acc / 200;
...
bool MsckfVio::resetCallback() {
  printf("Start resetting msckf vio...");
  ...
  double gyro_bias_cov, acc_bias_cov, velocity_cov;
  YAML::Node initial_covariance = input_params_["initial_covariance"];
  velocity_cov = initial_covariance["velocity"].as<double>();    // Defaul 0.25
  gyro_bias_cov = initial_covariance["gyro_bias"].as<double>();  // Defaul 1e-4
  acc_bias_cov = initial_covariance["acc_bias"].as<double>();    // Defaul 1e-2
  ...
  printf("Resetting msckf vio completed...");
  return true;
}

double MsckfVio::GetTimeSec() const {
...
void MsckfVio::featureCallback(const struct ImagePoints& msg) {
  ...
  double processing_start_time = GetTimeSec();
  ...
  stateAugmentation(msg.TimestampToSec());
  double state_augmentation_time = GetTimeSec() - start_time;

  // Add new observations for existing features or new
  // features in the map server.
  start_time = GetTimeSec();
  addFeatureObservations(msg);
  double add_observations_time = GetTimeSec() - start_time;

  // Perform measurement update if necessary.
  start_time = GetTimeSec();
  removeLostFeatures();
  double remove_lost_features_time = GetTimeSec() - start_time;

  start_time = GetTimeSec();
  pruneCamStateBuffer();
  double prune_cam_states_time = GetTimeSec() - start_time;

  // Publish the odometry.
  start_time = GetTimeSec();
  // TODO output the odometery data
  publish();
  double publish_time = GetTimeSec() - start_time;

  // Reset the system if necessary.
  onlineReset();

  double processing_end_time = GetTimeSec();
  double processing_time = processing_end_time - processing_start_time;
  if (processing_time > 1.0 / frame_rate) {
    ++critical_time_cntr;
    printf("\033[1;31mTotal processing time %f/%d...\033[0m", processing_time, critical_time_cntr);
...
void MsckfVio::batchImuProcessing(const uint64_t& time_bound) {
  ...
  while (imu_msg_buffer.front().timestamp_us <= time_bound) {
    ...
    m_gyro << imu_msg.gyroXYZ[0], imu_msg.gyroXYZ[1], imu_msg.gyroXYZ[2];
    m_acc << imu_msg.accelXYZ[0], imu_msg.accelXYZ[1], imu_msg.accelXYZ[2];
...
void MsckfVio::addFeatureObservations(const ImagePoints& msg) {
  ...
  // Add new observations for existing features or new
  // features in the map server.
  for (const auto& feature : msg.pts) {
    if (map_server.find(feature.id) == map_server.end()) {
      ...
      map_server[feature.id].observations[state_id] =
          Vector4d(feature.cam0_pt[0], feature.cam0_pt[1], feature.cam1_pt[0], feature.cam1_pt[1]);
    } else {
...
void MsckfVio::removeLostFeatures() {
  ...
  for (auto iter = map_server.begin(); iter != map_server.end(); ++iter) {
    ...
    if (feature.observations.find(state_server.imu_state.id) != feature.observations.end()) continue;
  ...
  for (const auto& feature_id : processed_feature_ids) {
    ...
    for (const auto& measurement : feature.observations) cam_state_ids.push_back(measurement.first);
...
void MsckfVio::pruneCamStateBuffer() {
  ...
  for (auto& item : map_server) {
    ...
    for (const auto& cam_id : rm_cam_state_ids) {
      if (feature.observations.find(cam_id) != feature.observations.end()) involved_cam_state_ids.push_back(cam_id);
    }
