...
bool MsckfVio::loadParameters(const YAML::Node &input_params) {
  ...
  Feature::optimization_config.translation_threshold = input_params[
    "feature_translation_threshold"].as<double>();  // Default 0.2;
  ...
  Eigen::Matrix3d R_imu_cam0_euler =
    (AngleAxisd(R_imu_cam0_euler_v[0], Vector3d::UnitX())
    * AngleAxisd(R_imu_cam0_euler_v[1],  Vector3d::UnitY())
    * AngleAxisd(R_imu_cam0_euler_v[2], Vector3d::UnitZ())).toRotationMatrix();
...
double MsckfVio::GetTimeSec() const {
  return static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::
    steady_clock::now().time_since_epoch()).count()) / 1.0E6;
}
...
void MsckfVio::measurementJacobian(const StateIDType& cam_state_id,
