...
ImageSink::ImageSink(const std::string_view sink_pipeline, cv::Size frame_size, int framerate) : sink_pipeline_(sink_pipeline), frame_size_(frame_size), framerate_(framerate) {}

ImageSink::ImageSink(const YAML::Node& config) : ImageSink(config["sink_pipeline"].as<std::string>(), cv::Size(config["width"].as<int>(), config["height"].as<int>()), config["framerate"].as<int>()) {}
...
int ImageSink::SendFrame(cv::Mat &frame) {
  if (!cam_sink_) {
...
int ImageSink::SendFrame(cv::cuda::GpuMat &d_frame) {
  cv::Mat frame;
